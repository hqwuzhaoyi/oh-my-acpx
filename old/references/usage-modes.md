...
在 OpenClaw 平台内，用 `sessions_spawn` / `sessions_yield` 编排，`.oma/plans/plan.json` 做跨 turn 持久记忆：

```bash
SID="plan-$(date +%s)"

# Turn 1: 分析需求，创建 .oma/plans/plan.json
openclaw agent --agent claude --session-id "$SID" \
  --message "分析以下需求，创建 .oma/plans/plan.json：<需求描述>。
...
# Turn 2+: 逐个执行 story
openclaw agent --agent claude --session-id "$SID" \
  --message "读取 .oma/plans/plan.json，找最高优先级 pending story，
用 sessions_spawn 执行，完成后更新 .oma/plans/plan.json。
...
在 Claude Code / Gemini CLI 会话中，SKILL.md 指导 agent 自己管理 `.oma/plans/plan.json`：

1. 收到用户需求后，先分析并创建 `.oma/plans/plan.json`
2. 用 `Agent` tool 或 `Bash` 分派子任务给对应 agent
3. 每完成一个 story，读 `.oma/plans/plan.json` → 更新状态 → 找下一个
...
for i in $(seq 1 $MAX_ITERATIONS); do
  NEXT=$(node -e "
    const p = require('./.oma/plans/plan.json');
  ...
  node -e "
    const fs = require('fs');
    const p = JSON.parse(fs.readFileSync('.oma/plans/plan.json'));
    const s = p.stories.find(s => s.id === '$SID');
    s.passes = true;
    if (p.stories.every(s => s.passes)) console.log('ALL_COMPLETE');
    fs.writeFileSync('.oma/plans/plan.json', JSON.stringify(p, null, 2));
  "
done
```
