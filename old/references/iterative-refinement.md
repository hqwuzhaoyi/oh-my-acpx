...
Multi-round generate-evaluate cycles for high-risk stories marked `iterative:true` in the active plan file (typically `.oma/plans/plan.json`). Depends on generator-evaluator for each round's scoring.

**Audience:** acp-orchestrator handling stories with `"iterative": true` in the active plan file (typically `.oma/plans/plan.json`).
...
Mark `"iterative": true` in the plan file when any of these apply:
