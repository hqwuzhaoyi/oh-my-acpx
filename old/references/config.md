...
```json
{
  ...
  "agents": {
    ...
    "gemini": {
      "command": "env GEMINI_CLI_HOME=/Users/admin/.gemini-acpx gemini --acp"
    }
  }
}
```
...
- 如果 Gemini hooks/skills 污染 ACP JSON 输出，给 `acpx` 单独配置 `GEMINI_CLI_HOME`，并在该目录下放一份无 hooks 的 `.gemini/settings.json`
- 已验证可用的本机配置：

```bash
mkdir -p /Users/admin/.gemini-acpx/.gemini
ln -sfn /Users/admin/.gemini/oauth_creds.json /Users/admin/.gemini-acpx/.gemini/oauth_creds.json
ln -sfn /Users/admin/.gemini/google_accounts.json /Users/admin/.gemini-acpx/.gemini/google_accounts.json
```
...
```bash
acpx --approve-all --timeout 90 gemini exec "Reply exactly: ACPX_GEMINI_HOME_OK"
```

- 调试 Gemini ACP 时可直接运行：

```bash
env GEMINI_CLI_HOME=/Users/admin/.gemini-acpx gemini --acp --debug
```
