...
**ACP 实战备注**:
```bash
# 当全局 ~/.gemini/settings.json 里有 hooks/skills，可能污染 ACP JSON 输出
# 可给 acpx 单独指定一份无 hooks 的 Gemini 配置目录
env GEMINI_CLI_HOME=/Users/admin/.gemini-acpx gemini --acp
```

```bash
# 如需调试 Gemini ACP 启动/握手过程，可直接打开 debug
